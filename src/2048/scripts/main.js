...
function initGame() {
  gameState.grid = Array(CONFIG.gridSize)
    .fill()
    .map(() => Array(CONFIG.gridSize).fill(0));
  gameState.score = 0;
  updateScore();
...
function move(direction) {
  ...
  function pushNumbers(arr) {
    ...
    while (newArr.length < CONFIG.gridSize) {
      newArr.push(0);
    }
    return newArr;
  }
...
function checkGameOver() {
  for (let i = 0; i < CONFIG.gridSize; i++) {
    for (let j = 0; j < CONFIG.gridSize; j++) {
      ...
      if (
        i < CONFIG.gridSize - 1 &&
        gameState.grid[i][j] === gameState.grid[i + 1][j]
      )
        return false;
      if (
        j < CONFIG.gridSize - 1 &&
        gameState.grid[i][j] === gameState.grid[i][j + 1]
      )
        return false;
    }
  }
...
function showLeaderboard() {
  ...
  const validScores = gameState.bestScores.filter((score) => {
    return (
      (!score.score_text || score.score_text.includes(score.score_numeric)) &&
...
function getUsernameUrl(userHandle) {
  if (!userHandle || userHandle.startsWith("anon_"))
    return "`An anonymous user`";
  return `[\`@${userHandle}\`](<https://www.wildwest.gg/u/${userHandle}>)`;
}

function getWebhookUrl(id) {
  return id === "play" ? CONFIG.notificationUrl : CONFIG.webhookUrl;
}
...
window.addEventListener("message", (e) => {
  const { type, scores, handle } = e.data;
  switch (type) {
