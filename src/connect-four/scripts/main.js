...
function handleHover(e) {
	if (e.target.classList.contains('cell-wrapper') || e.target.classList.contains('cell')) {
		...
		if (hoverRow !== -1) {
			...
			hoverCell.classList.add(gameState.currentPlayer === 1 ? 'hover-red' : 'hover-yellow');
		}
	}
}
...
function checkWin(row, col) {
	...
	for (let [dx, dy] of directions) {
		...
		for (let i = 1; i < GG_ALL_GAME_CONFIG.winLength; i++) {
			...
			if (newRow < 0 || newRow >= GG_ALL_GAME_CONFIG.rows || newCol < 0 || newCol >= GG_ALL_GAME_CONFIG.cols || gameState.board[newRow][newCol] !== gameState.currentPlayer) break;
			count++;
		}
		for (let i = 1; i < GG_ALL_GAME_CONFIG.winLength; i++) {
			const newRow = row - i * dx;
...
function handleClick(e) {
	...
	if (checkWin(row, col)) {
		document.getElementById('message').textContent = `Player ${gameState.currentPlayer} wins!`;
		...
		document.getElementById('message').textContent = `Player ${gameState.currentPlayer}`;
...
function resetGame() {
	gameState.board = Array(GG_ALL_GAME_CONFIG.rows).fill().map(() => Array(GG_ALL_GAME_CONFIG.cols).fill(0));
...
function updateCellSize() {
	const width = (window.innerWidth - 70) / (GG_ALL_GAME_CONFIG.cols * 1.15);
	const height = (window.innerHeight - 270) / (GG_ALL_GAME_CONFIG.rows * 1.15);
