...
function initializeHeaps() {
  gameState.heaps = Array(HEAP_COUNT)
    .fill(0)
    .map(() => random(MIN_STONES, MAX_STONES));
  updateHeapsDisplay();
}
...
function updateHeapsDisplay() {
  ...
  displays.forEach((display) => {
    display.innerHTML = gameState.heaps
      .map(
        (count, index) => `
			<div class="heap ${count > 0 ? "selectable" : ""} ${
          gameState.selectedHeap === index ? "selected" : ""
        }" data-heap="${index}">
...
function updateStonesSelectedText(amount) {
  ...
  stonesSelected.textContent = `${amount} ${
    amount === "1" ? "stone" : "stones"
  } selected`;
}
...
function checkGameOver() {
  if (!gameState.heaps.some((h) => h > 0)) {
    ...
    if (gameState.gameMode === "bot") {
      winner.textContent =
        gameState.currentPlayer === 0 ? "You win!" : "Bot wins!";
    } else {
...
document
  .getElementById("go-first")
  .addEventListener("click", () => setFirstPlayer(true));
document
  .getElementById("go-second")
  .addEventListener("click", () => setFirstPlayer(false));
