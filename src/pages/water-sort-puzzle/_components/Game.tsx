...
function App() {
  onMount(() => {
    ...
    const gameState = {
      ...
      tubes: [] as Tube[],
      selectedTube: null as number | null,
    };
    ...
    function createTubes() {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        for (
          let j = 0;
          j < tubesPerRow && i * tubesPerRow + j < tubeCount;
          j++
        ) {
    ...
    function pourLiquid(from: number, to: number) {
      const colorToPour =
        gameState.tubes[from]![gameState.tubes[from]!.length - 1];
      while (
    ...
    function updateTubeSize() {
      ...
      const newControlButtonSize = Math.max(
        24,
        Math.min(54, windowWidth * 0.12)
      );
  ...
  return (
    <>
      <div id="game-container">
        <div id="top-menu">
          <button id="previous-level" class="control-button"></button>
          <span id="currrent-level">Level 1</span>
          <button id="next-level" class="control-button"></button>
          <button id="restart-level" class="control-button"></button>
        </div>
