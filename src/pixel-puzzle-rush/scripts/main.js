...
function loadData() {
  gameState.bestTimes =
    JSON.parse(localStorage.getItem("pixel-puzzle-rush-data") ?? "{}")
      ?.bestTimes ?? {};
}
...
function startTimer() {
  gameState.timerInterval = setInterval(() => {
    ...
    timerElement.textContent = `Time: ${(gameState.currentTime / 1000).toFixed(
      2
    )}s`;
  }, 10);
}
...
function checkMatch() {
  if (
    gameState.grid.every((color, index) => color === gameState.pattern[index])
  ) {
    endGame();
  }
}
...
function endGame() {
  ...
  const key = `${gameState.gridSize}x${gameState.gridSize}-${gameState.colors.length}`;
  if (
    !gameState.bestTimes[key] ||
    gameState.currentTime < gameState.bestTimes[key]
  ) {
    gameState.bestTimes[key] = gameState.currentTime;
    updateBestTime();
    saveData();
    messageElement.textContent = `New Best Time for ${key}: ${(
      gameState.currentTime / 1000
    ).toFixed(2)}s!`;
  } else {
    messageElement.textContent = `Puzzle Solved! Time: ${(
      gameState.currentTime / 1000
    ).toFixed(2)}s`;
  }
...
gridElement.addEventListener("click", (e) => {
  ...
  if (index !== undefined) {
    const nextColorIndex =
      (gameState.colors.indexOf(gameState.grid[index]) + 1) %
      gameState.colors.length;
...
document
  .getElementById("color-count")
  .addEventListener("change", updateBestTime);
