...
function saveData() {
	localStorage.setItem('water-sort-puzzle-data', JSON.stringify({
...
function loadData() {
	...
	document.getElementById('currrent-level').textContent = `Level ${gameState.currentLevel + 1}`;
}
...
async function getFinalMessage() {
	const response = await fetch(`${GG_ALL_GAME_CONFIG.dataUrl}/finalmessage.html?timestamp=${Date.now()}`);
	GG_ALL_GAME_CONFIG.finalMessage = await response.text();
}
...
function loadLevel() {
	...
	renderTubes();
	document.getElementById('currrent-level').textContent = `Level ${gameState.currentLevel + 1}`;
	document.getElementById('previous-level').disabled = (gameState.currentLevel === 0);
	document.getElementById('next-level').disabled = (gameState.currentLevel === GG_ALL_GAME_CONFIG.levels.length - 1);
}
...
function updateTubeSize() {
	...
	document.documentElement.style.setProperty('--tube-width', `${newTubeWidth}px`);
	document.documentElement.style.setProperty('--tube-height', `${newTubeHeight}px`);
	document.documentElement.style.setProperty('--tubes-gap', `${newTubesGap}px`);
	document.documentElement.style.setProperty('--tube-border-radius', `${newTubeBorderRadius}px`);
	...
	document.documentElement.style.setProperty('--top-menu-font-size', `${newTopMenuFontSize}px`);
	document.documentElement.style.setProperty('--control-button-size', `${newControlButtonSize}px`);
	document.documentElement.style.setProperty('--top-menu-gap', `${newTopMenuGap}px`);
}
...
document.getElementById('next-level').addEventListener('click', () => {
	if (gameState.currentLevel < GG_ALL_GAME_CONFIG.levels.length - 1) {
		gameState.currentLevel++;
		saveData();
		loadLevel();
	}
